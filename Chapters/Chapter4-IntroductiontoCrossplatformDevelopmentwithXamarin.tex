% Chapter 4
% !TEX root = ../main.tex
\chapter{Cross Platform Development with Xamarin} % Main chapter title
...
TBD done how  to set up recommended and setup
...
\begin{tabular}{l l l l}
\toprule
\tabhead {System} & \tabhead {RECOMMENDED} & \tabhead {NOTES} \\
\midrule

    \textbf{Operating System} & Windows 10 & \multicolumn{1}[l] {p{65.085em}}{The minimum operating system version is Windows 7. Xamarin.Forms Windows support requires Windows 8.1, and Xamarin.Forms UWP support requires Windows 10.} \\
    \textbf{Xamarin.iOS} & iOS 10 SDK & To build iOS projects on Windows requires: \\
          & installed on a Mac & \multicolumn{1}[l] {p{65.085em}}{Visual Studio 2013 or newer, and a Mac computer, network-accessible from the Windows computer, that conforms to the minimum requirements for running Xamarin on macOS (above).} \\
    \textbf{Xamarin.Android} & Android 6.0 / API level 23 & \multicolumn{1} [l]  {p{65.085em}}{You can still target older Android versions while using the latest SDK, or you can build against older versions of the SDK if required.} \\
    \textbf{Xamarin.Forms} & \multicolumn{1}[l] {p{47em}}{Xamarin.Forms apps for iOS and Android
can be deployed to the relevant platforms as described above. } & NA \\
  ...
  \end{tabular}
  \end{table}
%https://dzone.com/articles/xamarin-the-good-the-bad-and-the-ugly-1
